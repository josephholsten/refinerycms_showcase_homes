$(document).ready(function(){
  $('#custom_images_tab a').click(function(){
    if (!(picker = $('#showcase_home_image_picker')).data('size-applied')){
  ...
  $('#content #showcase_home_images li textarea:hidden').each(function(index) {
...
reset_functionality = function() {
  ...
  $('#content #showcase_home_images li:not(.empty)').live('hover', function(e) {
    if (e.type == 'mouseenter' || e.type == 'mouseover') {
      if ((image_actions = $(this).find('.image_actions')).length == 0) {
        ...
        if ($(this).find('textarea.showcase_home_caption').length > 0) {
...
image_added = function(image) {
  ...
  new_list_item.appendTo($('#showcase_home_images'));
  reset_functionality();
}
...
reindex_images = function() {
  $('#showcase_home_images li textarea:hidden').each(function(i, input){
  ...
  $('#showcase_home_images li input:hidden:not(.caption)').each(function(i, input){
    // make the image's name consistent with its position.
  ...
  $('#showcase_home_images li input.caption:hidden').each(function(i, input){
    // make the image's name consistent with its position.
    parts = $(input).attr('name').split(']');
    parts[1] = ('[' + i);
    $(input).attr('name', parts.join(']'));
